    ...
    if serverName in serversList:
        ...
        if matchChannel(channelName, channelsList):
            ...
            if message.attachments:
                    ...
                    toSend = f"#{channelName}\nNickname: {message.author.name}\n{message.content}\n"
                    print(f"[+] Message: {toSend}")
                    toSend = removeTags(toSend)
                    if isPhoto(attachmentUrl):
                        url = f"{baseUrl}/sendPhoto?photo={attachmentUrl}&chat_id={config.TELEGRAM_RECEIVER_CHAT_ID}&caption={toSend}"
                        print(url)
                        sendMsg(url)
                    elif isVideo(attachmentUrl):
                        url = f"{baseUrl}/sendVideo?video={attachmentUrl}&chat_id={config.TELEGRAM_RECEIVER_CHAT_ID}&caption={toSend}"
                        sendMsg(url)
                    elif isDoc(attachmentUrl):
                        url = f"{baseUrl}/sendDocument?document={attachmentUrl}&chat_id={config.TELEGRAM_RECEIVER_CHAT_ID}&caption={toSend}"
                        sendMsg(url)
